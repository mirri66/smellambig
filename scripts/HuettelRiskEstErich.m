...
function [b_fit,a_fit]=HuettelRiskEstErich(subjNo, numTrialsPer, iter)
...
riskTrials = [];
ridx = [ones(numTrialsPer,1); zeros(numTrialsPer,1)];
riskIdx = Shuffle(ridx);
fprintf('ridx: %g \n', size(riskIdx))
...
% find array of relevant trials
ambigTrials = [];
aidx = [ones(numTrialsPer,1); zeros(numTrialsPer,1)];
ambigIdx = Shuffle(aidx);
fprintf('aidx: %g \n', size(ambigIdx))
